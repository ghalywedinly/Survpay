...
export function Thead({ children }: { children: React.ReactNode }) {
  return <thead className="border-b border-ink-100 text-start text-xs font-semibold uppercase tracking-wide text-ink-400">{children}</thead>;
}

export function Th({ children, className }: { children?: React.ReactNode; className?: string }) {
  return <th className={clsx('py-3 pe-4 text-start font-semibold', className)}>{children}</th>;
}
...
export function Tr({ children, className }: { children: React.ReactNode; className?: string }) {
  return <tr className={clsx('transition-colors hover:bg-ink-50/70', className)}>{children}</tr>;
}
